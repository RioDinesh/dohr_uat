...
module.exports = {
  ...
  GetTickBoxIsPresent: (data, callback) => {
    pool.query("select * from dh_customer where is_active=1 AND id=?", [data.id], (error, result, fields) => {
      if (error) {
        return callback(error);
      } else {
        return callback(null, result);
  ...
  GetTickBox: (callback) => {
    pool.query("select * from dh_description_tickbox_master where is_active=1", [], (error, result, fields) => {
      if (error) {
        return callback(error);
      } else {
        return callback(null, result);
  ...
  GetContactUs: (callback) => {
    pool.query("select * from dh_dohr_contactUs where is_active=1", [], (error, result, fields) => {
      if (error) {
        return callback(error);
      } else {
        return callback(null, result);
  ...
  GetCustomerDetailsForContactUs: (data,callback) => {
    pool.query("select * from dh_customer where email_id=?", [data.email], (error, result, fields) => {
      if (error) {
        return callback(error);
      } else {
        return callback(null, result);
  ...
  GetConsultantDetailsForContactUs: (data,callback) => {
    pool.query("select * from dh_substitute_consultant where email_id=?", [data.email], (error, result, fields) => {
      if (error) {
        return callback(error);
      } else {
        return callback(null, result);
  ...
  GetCustomerDetails: (data,callback) => {
    pool.query("select * from dh_customer where is_active=1", [], (error, result, fields) => {
      if (error) {
        return callback(error);
      } else {
        return callback(null, result);
  ...
  GetConsultantDetails: (data,callback) => {
    pool.query("select * from dh_substitute_consultant where is_active=1", [], (error, result, fields) => {
      if (error) {
        return callback(error);
      } else {
        return callback(null, result);
  ...
  GetExpressPassUserDetails: (data,callback) => {
    pool.query("select A.*,B.* from dh_express_pass_users A left join dh_substitute_consultant B on A.cons_id=B.id where A.is_active=1 AND A.pass_id=?", [data.pass_id], (error, result, fields) => {
      if (error) {
        return callback(error);
      } else {
        return callback(null, result);
  ...
  GetVacancyCanceledUserDetails: (data,callback) => {
    pool.query("select A.*, A.created_at as vacancy_created_at,B.* from dh_canceled_vacancy_info A left join dh_substitute_consultant B on A.consultant_id=B.id where A.is_active=1 AND A.vacancy_id=? ORDER BY A.id DESC LIMIT 1", [data.vacancy_id], (error, result, fields) => {
      if (error) {
        return callback(error);
      } else {
        return callback(null, result);

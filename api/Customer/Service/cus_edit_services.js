...
module.exports = {
  ApproveMyVacancy: (data, callback) => {
    pool.query(
      "update dh_vacancy_new set vacancy_status=? where id=?",
      [data.status, data.id],
      (error, result, fields) => {
        if (error) {
  ...
  Approved_vacancy_req: (data, callback) => {
    if (data.save_cons == true) {
      pool.query(
        "update dh_vacancy_new set approved_stime=?,approved_etime=?,feedback=?,total_approved_hrs=?,report_break_time=?,vacancy_is_approved=1 where id=?",
        [
          data.approved_stime,
          data.approved_etime,
          data.feedback,
  ...
  DenyMyVacancy: (data, callback) => {
    pool.query(
      "update dh_vacancy_new set vacancy_status=0 ,assigned_to_internal=0 where id=?",
      [data.id],
      (error, result1, fields) => {
        if (error) {
          return callback(error);
          ...
          pool.query(
            "update dh_uncovered_management set is_covered=0  where id=?",
            [data.uncovered_id],
            (err, result2, fields) => {
              if (err) {
                return callback(err);
                ...
                pool.query(
                  "insert into deny_request_list(cus_id,vid,reason) values(?,?,?)",
                  [data.cus_id, data.id, data.reason],
  ...
  Logout: (data, callback) => {
    if (data.type == 1) {
      pool.query(
        "update dh_customer set notification_id='NA' where id=?",
        [data.cus_id],
        (err, result, fields) => {
          if (err) {
            return callback(err);
    ...
    if (data.type == 2) {
      pool.query(
        "update dh_substitute_consultant set notification_id='NA' where id=?",
        [data.cons_id],
        (err, result, fields) => {
          if (err) {
            return callback(err);

...
module.exports = {
  ...
  FCM_MESSAGE: function Fcm_Messaging(message) {
    const options = {
      headers: {
        ...
        "Authorization": "key=AAAAHq4_tgs:APA91bHZFGIfqiA5H7uRmYOnlMWcelZ8Gp-atLxhGj1NMLT6nYVg1CCO77fb_19cULtdT0XSM3zfnIqTTY7JfF2iOIfKT3biaB0rYzxZU0E0Hyc4by-XjBWIaRQKfC_nMdorpyIN3z4g"
      }
    };
    axios.post('https://fcm.googleapis.com/fcm/send', message, options)
      .then((response) => {
        //receive response
  ...
  Check30DaysPassed:function isCoercedTo30Days(year, month, day) {
    ...
    const coercedDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), 30);
  
    return currentDate.getMonth() === coercedDate.getMonth() &&
 ...
 IsNOtPassed30Days: function isDatePassed30Days(dateString) {
  ...
  GetExtensionType:  function getFileExtensionFromBase64(base64String) {
    ...
    const regexResult = /^data:([a-zA-Z0-9]+\/[a-zA-Z0-9]+);base64,/.exec(base64String);
